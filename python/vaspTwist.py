...
def prompt_selection(candidates):
    ...
    total_poscars = sum(
        len(get_shift_grid(detect_lattice_type(c["bilayer_lattice_matrix"])))
        for c in candidates
    )
...
def write_output_list(filepath, output_records):
    ...
    with open(filepath, 'w') as o:
        ...
        o.write("#\n")
        o.write(f"# {'Path':<60}  {'Theta(deg)':>10}  {'Atoms':>6}"
                f"  {'Stacking':<10}  {'Strain(%)':>10}\n")
        o.write("# " + "-" * 104 + "\n")
        for rec in output_records:
            o.write(f"  {rec['path']:<60}  {rec['theta']:>10.4f}  {rec['total_atoms']:>6}"
                    f"  {rec['stacking']:<10}  {rec['strain_pct']:>10.4f}\n")
    
    with open('twist_list.dat', 'w') as o:
        for rec in output_records:
            o.write(f"{rec['path']}\n")
...
def main():
    ...
    moire_vectors = find_moire_vectors(bottom["lattice_matrix"], top["lattice_matrix"], 0.0, 180.0,
                                        THETA_STEP, N_MIN, n_max)
    ...
    with mp.Pool(processes=num_cores) as pool:
        per_theta_results = pool.starmap( build_candidates_for_theta, 
                                         [(theta_key, vec_list, bottom["lattice_matrix"], bottom["positions_cartesian"],
                                           bottom["species"], bottom["selective_dynamics"], bottom["flags"],
                                           top["lattice_matrix"], top["positions_cartesian"], top["species"],
                                           top["selective_dynamics"], top["flags"], sort_elements, known_elements)
    ...
    for index in chosen_indices:
        ...
        base_dir = os.path.join(working_dir, f"{index + 1}_twist_{theta:.4f}deg_{total_atoms}atoms")
    ...
    list_path = os.path.join(working_dir, "output_list.dat")
    write_output_list(list_path, output_records)
    
    print(f"\nFinished! Written {written_count} POSCAR(s).\n")
    print(f"Output list written to {list_path}\n")
